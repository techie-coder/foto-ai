...
export const ClickableImage = ({ modelProps, handleModel }: { modelProps: Model, handleModel: (model: Model) => void }) => {
    ...
    return (
        <div className="w-[70px] h-[70px] relative">
            <button className='absolute top-0 left-0 w-full h-full bg-black/50 rounded-md' onClick={() => { setClick(!click); console.log(click); handleModel(modelProps) }}>{click && (<Image src={tick} fill alt="tick" />)}
                <Image
