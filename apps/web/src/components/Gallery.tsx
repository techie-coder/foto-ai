        ...
        <div className="grid grid-cols-4 gap-5 p-5">
            {images.length > 0 && images.map((image) => (
                <div key={image.id} className="relative h-[250px] w-[250px] rounded-lg overflow-hidden group">
                    {image.status === "Pending" ? <Skeleton className="w-full h-full rounded-md dark" /> : (<>
                        ...
                        /></button>
                        <div className="absolute bottom-0 h-full bg-black/30 opacity-0 transition group-hover:opacity-80 duration-300 ease-in-out flex flex-col items-center justify-center">
